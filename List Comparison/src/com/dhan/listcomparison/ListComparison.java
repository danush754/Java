package com.dhan.listcomparison;
...
//	this method initially fills the the list with certain amount of elements that the user requested
	private static void fillOutList(List<Integer> list, int amoutOfElements ) {
		for(int i = 0;i < amoutOfElements ;i++) {
			list.add(i);
		}
	}
	
//	this method adds up 100 new elements to the start of the list 
...
//	this method adds up 100 new elements in the middle of the list
	public 	static void addElementsToMiddle(List<Integer> list, int numberOfElementsToAdd) {
		int midElement = Math.round(list.size()/2);
		int stopVal = midElement+ numberOfElementsToAdd;
		for(int i = 0;i < stopVal;i++ ) {
			list.add(i,midElement);
			midElement++;
		}
	}
	
	public static void addElementsToEnd(List<Integer> list, int numberOfElementsToAdd) {
		int lastVal = list.size();
		int stopVal = list.size() + numberOfElementsToAdd;
		for(int i = 0;i < stopVal;i++) {
			list.add(i,lastVal);
			stopVal++;
		}
	...
	for(int i = 0 ; loopIncrementer < numberOfElementsToRemove ; i = i ) {
		list.remove(i);
		loopIncrementer++;
	}
}
    ...
    public static void removeElementsFromMiddle(List<Integer> list,int numberOfElementsToRemove) {
    	int midElement = Math.round(list.size()/2);
    	int stopVal = midElement + numberOfElementsToRemove;
    	int loopIncrementer = midElement;
    	for(int i = midElement ; loopIncrementer <= stopVal; i = midElement) {
    		list.remove(i);
    		loopIncrementer++;
    	}
    ...
    public static void removeElementsFromEnd(List<Integer> list,int numberOfElementsToRemove) {
		int lastVal = list.size();
		int stopVal = lastVal - numberOfElementsToRemove;
		for(int i = lastVal; i > stopVal;i--) {
			list.remove(i);
		}
	...
	public static void main(String[] args) {
		...
		fillOutList(linkedList, 1_000);
		fillOutList(arrayList, 1_000);
		
		System.out.println("**************** Array List *****************");
		
		long mill = System.nanoTime();
		addElementsToBeginning(arrayList, 100);
		long add100ElementToList = (System.nanoTime() - mill) / 10000;
		
		System.out.println("Added 100 new elements to the beginning of the list :" + add100ElementToList);
